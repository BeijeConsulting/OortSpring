...
public class BookController {
	...
	@RequestMapping(value = root + "/book/{id}", method = RequestMethod.GET)
	public String author(@PathVariable Integer id, Model model) {
	
		Book book = bookService.load(id);
				
		model.addAttribute("book", book);
		model.addAttribute("author", authorService.getAuthorNominative(book.getAuthorId()));
		model.addAttribute("publisher", publisherService.getPublisherName(book.getPublisherId()));
		
		return root + "/entity/book";
